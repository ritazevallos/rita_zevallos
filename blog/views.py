...
from ritazevallos.blog.models import Tag, Node, Path, PathNodeRelationship
...
def edit_path(request, path_id):
    if request.user.is_superuser:
        path = Path.get_object_or_404(Path, id=path_id)
...
def projects(request):
	tag = get_object_or_404(Tag, name="Projects")
	nodes = Node.objects.filter(tags__id = tag.id).order_by('date')
	return render(request, "nodes.html", {
		'title': tag.name,
		'nodes': nodes,})
